          ...
          <tbody>
            {clients.map((client) => (
              <tr key={client.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary">
                      {(client.name || "NA").split(" ").map((n) => n[0]).join("")}
                    </div>
                    <span className="text-sm font-medium text-foreground">{client.name || "N/A"}</span>
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-sm text-muted-foreground">{client.contractStart ? new Date(client.contractStart).toLocaleDateString() : "-"} - {client.contractEnd ? new Date(client.contractEnd).toLocaleDateString() : "-"}</td>
                <td className="px-4 py-3">
