...
export default function LeadsTable({ leads = [], showActions = true }) {
  return (
    <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          <tbody>
            {leads.map((lead) => (
              <tr key={lead.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary">
                      {(lead.name || "NA").split(" ").map((n) => n[0]).join("")}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-foreground">{lead.name || "N/A"}</p>
                ...
                <td className="px-4 py-3 text-sm text-muted-foreground">{lead.dateAdded ? new Date(lead.dateAdded).toLocaleDateString() : "-"}</td>
                {showActions && (
                  <td className="px-4 py-3">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-border">
        <p className="text-xs text-muted-foreground">Showing 1 to {leads.length} leads</p>
        <div className="flex gap-1">
          {[1, 2, 3, "...", 45].map((p, i) => (
            <button key={i} className={`w-8 h-8 rounded text-xs font-medium transition-colors ${p === 1 ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}>
              {p}
            </button>
          ))}
        </div>
