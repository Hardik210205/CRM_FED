          ...
          <tbody>
            {employees.map((emp) => (
              <tr key={emp.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary">
                      {(emp.name || "NA").split(" ").map((n) => n[0]).join("")}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-foreground">{emp.name || "N/A"}</p>
                ...
                <td className="px-4 py-3 text-sm text-foreground">{emp.leads ?? 0}</td>
                <td className="px-4 py-3 text-sm text-foreground">{emp.clients ?? 0}</td>
                <td className="px-4 py-3">
                  <Badge variant="outline" className={String(emp.status).toLowerCase() === "active" ? "bg-success/10 text-success border-success/20" : "bg-warning/10 text-warning border-warning/20"}>{emp.status || "-"}</Badge>
                </td>
                <td className="px-4 py-3">
